...
async function showMsg() {
	...
	if ($.joylevel) {
		ReturnMessage += `【汪汪乐园】汪汪${$.joylevel}级\n`;
	}
	if ($.JoyRunningAmount) {
...
function getCoupon() {
    return new Promise(resolve => {
        let options = {
            ...
            headers: {
                'authority': 'wq.jd.com',
                "User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;10.1.2;15.0;network/wifi;Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
                'accept': '*/*',
