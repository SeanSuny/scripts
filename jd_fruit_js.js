  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        ...
        if ($.isNode()) {
          await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName || $.UserName}水果已可领取`, `【京东账号${$.index}】${$.UserName}\n【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n请去京东APP或微信小程序查看`);
        }
        return
...
async function doTenWaterAgain() {
  ...
  */
  for (let i = 0; i < ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy) / 100; i++) {
    //使用快速浇水卡
    await userMyCardForFarm('fastCard');
    console.log(`使用快速浇水卡结果:${JSON.stringify($.userMyCardRes)}`);
    if ($.userMyCardRes.code === '0') {
